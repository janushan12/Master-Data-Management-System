...
$countSql = $is_admin
        ...
        : "SELECT COUNT(*) AS total FROM master_items WHERE user_id = $user_id";
...
$sql = $is_admin
        ? "SELECT i.*, b.name AS brand_name, c.name AS category_name FROM master_items i
...
// Apply search
if (!empty($search)) {
    $sql .= " AND (i.name LIKE '%$search%' OR i.code LIKE '%$search%')";
}

// Apply filter
if (!empty($statusFilter)) {
    $sql .= " AND i.status = '$statusFilter'";
}
...
<html>
    ...
    <body>
        <div class="sidebar">
            ...
            <a href="brand/list.php"><i class="bi bi-tags"></i> Brands</a>
                        ...
                        <div class="card-body">
                            ...
                            <form method="GET" class="row g-2 mb-3">
                                ...
                                <div class="col-md-2 d-flex gap-2">
                                    ...
                                    <a href="index.php" class="btn btn-secondary btn-sm w-100">Reset Filter</a>
                                </div>
                            </form>
                            <hr>
                                ...
                                <tbody>
                                    <?php if ($result->num_rows > 0) : ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><?php if ($row['attachment']) echo "<a href='uploads/".$row['attachment']."' target='_blank'>View</a>"; ?></td>
                                            <td><?= $row['status'] ?></td>
                                            <td>
                                                <a href="item/edit.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-warning">Edit</a> |
                                                <a href="item/delete.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Delete this brand?')">Delete</a>
                                            </td>
                                        </tr>
                                        <?php endwhile; ?>
                                    <?php else : ?>
